...
//    vec2 kBounds = sphereIntersection2(vec3(0.0, 0.0, 0.0), 2.2, origin, dir);
//    float k0 = kBounds.x;
...
//    int maxIter = 10;
//    int iter = 0;
//    while (iter<maxIter+int(u_Radius*0.01)/* && dk<10000.0*/) {
//        float dy = b-a;
//        dk = k1-k0;
//        float deriv = dy/dk;
//
...
vec4 rrS(vec2 pos, vec2 outPos) {
    ...
    if (objectIntersected) mixedCol *= vec4(2.0*u_ObjectColor.rgb, 1.0);
    else mixedCol *= vec4(2.0*u_BkgColor.rgb, 1.0);
//    float glowIntensity = mix(3.0/pow(minDist, 1.0), 1.0/pow(minDist, 5.0), clamp(0.0, 1.0, minDist/2.5));
    ...
    vec4 glowCol = mixedCol + vec4(u_GlowColor.rgb*glowIntensity, 0.0);
