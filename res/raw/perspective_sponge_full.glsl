...
float getDist(vec3 q) {
    return getDistAndGlow(q).x;
}


mat3 rayMarch(vec3 origin, vec3 dir) {
	float d = 0.0;
    int i = 0;
    vec3 current = origin;
    float glow = 0.0;
    while (i<MAX_ITER) {
        vec2 distGlow = getDistAndGlow(current);
...
vec4 sponge(vec2 pos, vec2 outPos) {
    ...
    mat3 intersectionGlow = rayMarch(origin, dir);
    ...
    if (intersection.x!=OOB) {
        ...
        if ((u_SourceColor.a!=0.0 && (u_SourceColor.r!=0.0 || u_SourceColor.g!=0.0 || u_SourceColor.b!=0.0))
                || u_Specular!=0.0) {
            light = rayMarch(intersection+lightDir*0.01, lightDir)[0].x==OOB ? 1.0 : 0.0;
