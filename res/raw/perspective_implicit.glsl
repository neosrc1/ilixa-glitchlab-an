...
//    return 0.01*(sqr(p.x-2.0)*sqr(p.x+2.0)+sqr(p.y-2.0)*sqr(p.y+2.0)+sqr(p.z-2.0)*sqr(p.z+2.0)
//        +3.0*(p.x*p.x*p.y*p.y+p.x*p.x*p.z*p.z+p.y*p.y*p.z*p.z)+6.0*p.x*p.y*p.z-10.0*(p.x*p.x+p.y*p.y+p.z*p.z)+22.0);
}
...
//    vec2 kBounds = sphereIntersection2(vec3(0.0, 0.0, 0.0), 0.5*(1.0+u_Radius*0.02), origin, dir);
//    float k0 = kBounds.x;
