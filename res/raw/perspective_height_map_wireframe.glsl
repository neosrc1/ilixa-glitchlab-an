...
//    if (dot(dir, lightPos-cameraPos)>length(lightPos-cameraPos)*0.99) return vec4(1.0, 1.0, 1.0, 1.0); // show light source
    ...
    if (heightMap) {
        do {
            ...
            p += step;
            k += dk;
            bool online = fmod(p.x, dk*gap)<dk || fmod(p.y, dk*gap)<dk;
            stop = online && (dz==0.0 || (k!=k1 && sign(dz)==-sign(prevDz)));
        } while (k<=k2 && !stop);
        vec2 pp = (p-step).xy;
    ...
    if (!stop) return getBackground(pos);
    ...
    if (shadowing!=0.0) {
        ...
        if (u_Specular!=0.0) {
            ...
            specular = (u_Specular<25.0?u_Specular*0.04:1.0) * pow(clamp(dot(dir, reflectLightDir), 0.0, 1.0), 10.0-u_Specular*0.1);
        }
    }
