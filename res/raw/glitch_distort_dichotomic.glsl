...
vec2 distort(vec2 pos, vec2 a, vec2 b, vec2 A, vec2 B, vec2 splits, vec4 rect, float intensity) {
    vec2 c = (a+b)/2.0;
    if (u_Mode<=1) {
    ...
    for(int j=0; j<u_Count; ++j) {
        ...
        for (float i=0.0; i+sPos<scale; ++i) {
            ...
            if (horSplit) {
                ...
                float regY = mix(regRect.y, regRect.w, 0.5);
                if (abs(Y-p.y)<u_Thickness*0.001) { border = true; break; }
                if (p.y<Y) { rect.w = Y; regRect.w = regY; ++splits.y; sPos += inverter*sscale; } else { rect.y = Y; regRect.y = regY; splits.y += 100.0; sPos += (1.0-inverter)*sscale; }
            }
            else {
                float X = mix(rect.x, rect.z, variability*withBias(rnd.x, bias.x)+0.5);
                float regX = mix(regRect.x, regRect.z, 0.5);
                if (abs(X-p.x)<u_Thickness*0.001) { border = true; break; }
                if (p.x<X) { rect.z = X; regRect.z = regX; ++splits.x; sPos += inverter*sscale; } else { rect.x = X; regRect.x = regX; splits.x += 100.0; sPos += (1.0-inverter)*sscale; }
            }
            horSplit = !horSplit;
        ...
        if (border) break;
        p = distort(p, rect.xy, rect.zw, regRect.xy, regRect.zw, splits, rect, intensity);
    }
