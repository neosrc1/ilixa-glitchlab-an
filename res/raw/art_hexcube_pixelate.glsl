...
vec4 hex(vec2 pos, vec2 outPos) {
    ...
    if (hex.y>=u_Thickness*0.005) {
        ...
        float a = hex.x;
        if (abs(a)>2.0*M_PI/3.0) pickCoord = hex.zw + Y*vec2(0.0, 0.5);
        else if (a<0.0) pickCoord = hex.zw + Y*0.5*vec2(-SQRT3_2, -0.5);
        else pickCoord = hex.zw + Y*0.5*vec2(SQRT3_2, -0.5);
//        if (hex.x<-M_PI/3.0) pickCoord = hex.zw + Y*0.5*vec2(-SQRT3_2, -0.5);
//        else if (hex.x<M_PI/3.0) pickCoord = hex.zw + Y*vec2(0.0, 0.5);
//        else pickCoord = hex.zw + Y*0.5*vec2(SQRT3_2, -0.5);

        absCoord = (u_InverseModelTransform * vec3(pickCoord, 1.0)).xy;
        outCol = texture2D(u_Tex0, proj0(absCoord));
//        outCol = vec4(hex.x, 0.0, 0.0, 1.0);
    }
    else {
